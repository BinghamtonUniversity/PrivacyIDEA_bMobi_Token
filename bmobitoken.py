...
DEFAULT_MOBILE_TITLE = _("PrivacyIDEA")
    ...
    **Step 2**:
    ...
    def _api_endpoint_post(cls, request_data):
        ...
        if all(k in request_data for k in ("fbtoken", "pubkey")):
            ...
            #apigateway = getParam(request_data, "apigateway")
            # get the token_obj for the given serial:
            token_obj = get_one_token(serial=serial,tokentype=cls.get_class_type())
            pubkey_obj = token_obj.get_tokeninfo(PUBLIC_KEY_SMARTPHONE)
            pubkey_pem = pubkey_obj.replace("-", "+").replace("_", "/")
            pubkey_pem = "-----BEGIN PUBLIC KEY-----\n{0!s}\n-----END PUBLIC KEY-----".format(pubkey_pem.strip().replace(" ", "+"))
            #comment
            #pubkey_obj = _build_verify_object(pubkey_obj)
            ...
            if challengeobject_list:
                ...
                for chal in challengeobject_list:
                    ...
                    try:
                        #pubkey_pem.verify(b32decode(signature),sign_data.encode("utf8"),padding.PKCS1v15(),hashes.SHA256())
	...
	#title = DEFAULT_MOBILE_TITLE
        ...
        tok = get_one_token(serial=self.token.serial, tokentype='bmobi')
        pem_privkey = tok.get_tokeninfo(PRIVATE_KEY_SERVER)
    ...
    def authenticate(self, passw, user=None, options=None):
        ...
        otp_counter = -1
        reply = None
        #allowed = getParam(options,"allowed",optional=False)
